...
Here is the verify pipeline's output — four primer pairs screened across three diverse human haplotypes. Cells show the predicted PCR product size(s): **green** = the correct amplicon, **red** = an off-target product, grey = **dropout** (a binding-site variant kills the product). The CYP2D6 paralog pair co-amplifies the CYP2D7 pseudogene (red), and the dropout pair fails **in one of these three haplotypes** — a common CYP2D6 exon-3 variant ([rs1058164](https://www.ncbi.nlm.nih.gov/snp/rs1058164)) sits under its forward primer's 3′ terminal base, so the assay silently works in two people and not the third. Exactly the failures a single-reference design would miss:

![Verify pipeline output — a primer × haplotype amplicon-size matrix](docs/img/verify_matrix.svg)

> [!NOTE]
> **Three haplotypes tell you a failure exists, not how common it is.** In the matrix above the dropout happens to land on the European haplotype, which invites the reading that rs1058164 is a European variant. It is not. Re-screening the same four pairs across **30 haplotypes** (6 per superpopulation) drops the pair out in **11 of 24 evaluable haplotypes — 46%**, in every superpopulation sampled: AFR 2/5, AMR 3/6, EAS 1/3, EUR 3/5, SAS 2/5. The variant is globally common, and a 3-haplotype panel simply lacks the resolution to show that. Sample size is a property of your evidence, not of the assay — scale the panel to the claim you want to make.

Try it: run the bundled demo (GAPDH across 3 HPRC haplotypes, ~40 s once the data is prepared) with `bash demo-design-pipeline/run_demo.sh`, then open the report it writes locally at `demo-design-pipeline/gapdh/report.html` (the verify matrix above is written the same way, to `demo-verify-pipeline/verify_matrix.html`) — both are generated by the demo run and aren't tracked in the repo.
...
Target (CHM13 region, GRCh38 region, or FASTA) → anchor on **CHM13 v2.0** → project onto each haplotype (the *expected homologous locus*) → mask variable sites → **Primer3** candidates → exhaustive genome-wide binding search per haplotype → **thermodynamic** dropout classification (3′-end aware) → pair into amplicons → per-haplotype status (`pass/dropout/off_target/multi_product/uncertain`) → rank by coverage + specificity → TSV/JSON + HTML report.
...
**Prepare the haplotypes** (download → md5 → `faidx` on the BGZF → projection anchor grid). The anchor grid takes ~4.5 min per ~3 Gb assembly (measured over 27 consecutive builds; see [`sizing.md`](docs/sizing.md)), so for the full subset run this unattended — it is resumable and logs to `hprc-r2/prepare.log`:
...
This produces `verify_matrix.html` (plus `verify.json` and `.tsv`): a matrix with one column per primer pair and one row per haplotype — haplotypes go on the vertical axis because that is the count that grows (3, 30, ~460), while primer pairs stay few. Each cell shows the predicted PCR product size(s) — **green** for the correct on-target amplicon, **red** for an off-target product, grey **dropout** where a binding-site variant kills the product, `?` where the locus cannot be projected, and `>100 binding sites` where a repeat-derived primer binds in too many places to score.

Each pair also carries a **summary** row — on-target coverage plus a count of each failure mode. At 3 haplotypes you can read the cells directly; at 30+ the matrix runs off the bottom of the screen and the summary is the verdict (the primer headers stay pinned to the top while you scroll). Coverage is measured over *evaluable* haplotypes (total − not-projectable) and credits a haplotype whose target amplifies even if it also throws an extra band — the same `on_target_coverage` the design pipeline ranks on, so the two pipelines report one quantity under one name. `verify.tsv` breaks the same numbers into sortable columns (`coverage`, `n_covered`, `n_dropout`, …) for ranking many pairs at once.
