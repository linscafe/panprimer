...
Glossary for the pangenome PCR primer design tool. Terms here are canonical: use them
in code (class/function names), CLI, config, and reports. This file is a glossary only —
no implementation details, no decisions (those live in the plan / ADRs).

## Core terms

- **Target locus** — the genomic region we want to amplify. Anchored on **CHM13 v2.0**,
  the primary coordinate system. All other inputs (GRCh38 coords, raw FASTA) are
  normalized to a CHM13 anchor before anything else happens.

- **Haplotype** — one phased assembly (hap1 or hap2) of one HPRC R2 individual. The
  atomic unit of evaluation: coverage and specificity are counted over haplotypes, not
  individuals.
...
- **Amplicon** — a predicted PCR product: a forward and reverse binding site on one
  haplotype in convergent orientation within the allowed product-size window.

- **Dropout** — a haplotype where the target *should* amplify but a SNP/indel under a
  primer (typically near the 3′ end) prevents it.
...
- **uncertain** — the locus is broken/unmappable in this assembly, or its projection is
  unreliable. Never counted as failure: excluded from the denominator and reported
  separately.
